...
type KelasKuliah = {
    id: number;
    kode_kelas: string;
    mataKuliah?: { kode_matkul: string; nama_matkul: string } | null;
...
const saveGrade = (submission: Submission) => {
    router.put(route('dosen.kelas-kuliah.tugas.pengumpulan.nilai', [props.kelasKuliah.id, props.tugas.id, submission.id]), { nilai: grade.value }, {
...
<template>
    ...
    <AppLayout :breadcrumbs="[
        { title: 'Kelas Kuliah', href: route('dosen.kelas-kuliah.index') },
        { title: props.kelasKuliah.kode_kelas, href: route('dosen.kelas-kuliah.show', props.kelasKuliah.id) },
        ...
        <div class="min-h-full bg-[#f6f5f4]">
            <div class="mx-auto flex w-full max-w-[1000px] flex-col gap-4 px-4 py-6 sm:px-6 lg:px-8">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <Link :href="route('dosen.kelas-kuliah.show', props.kelasKuliah.id)"><Button variant="outline" class="rounded-lg bg-white">Kembali</Button></Link>
                </div>
                ...
                <section class="rounded-xl border border-[#e6e6e6] bg-white p-6 shadow-sm">
                    ...
                    <dl class="mt-4 grid gap-4 sm:grid-cols-2">
                        <div><dt class="text-xs uppercase text-[#a39e98]">Judul</dt><dd class="text-[15px] font-medium text-black">{{ props.tugas.judul_tugas }}</dd></div>
                        <div><dt class="text-xs uppercase text-[#a39e98]">Kelas</dt><dd class="text-[15px] font-medium text-black">{{ props.kelasKuliah.kode_kelas }} · {{ (props.kelasKuliah.mataKuliah ?? props.kelasKuliah.mata_kuliah)?.nama_matkul ?? '-' }}</dd></div>
                        <div><dt class="text-xs uppercase text-[#a39e98]">Tenggat Waktu</dt><dd class="text-[15px] text-[#31302e]">{{ formatDate(props.tugas.tenggat_waktu) }}</dd></div>
                        <div><dt class="text-xs uppercase text-[#a39e98]">Diunggah Oleh</dt><dd class="text-[15px] text-[#31302e]">{{ props.tugas.uploader?.name ?? '-' }}</dd></div>
                    </dl>
                ...
                <section class="rounded-xl border border-[#e6e6e6] bg-white p-6 shadow-sm">
                    ...
                    <div v-if="page.props.flash?.success" class="mt-4 rounded-xl border border-[#e6e6e6] px-4 py-3 text-sm text-[#1aae39]">{{ page.props.flash.success }}</div>
                    <div v-if="page.props.flash?.error" class="mt-4 rounded-xl border border-[#e6e6e6] px-4 py-3 text-sm text-[#dd5b00]">{{ page.props.flash.error }}</div>
                    <div class="mt-4 overflow-hidden rounded-xl border border-[#e6e6e6] overflow-x-auto">
                        <table class="w-full min-w-[760px] text-left">
                            <thead><tr class="border-b border-[#e6e6e6] bg-[#f6f5f4]"><th class="px-4 py-3 text-xs uppercase text-[#a39e98]">No.</th><th class="px-4 py-3 text-xs uppercase text-[#a39e98]">Mahasiswa</th><th class="px-4 py-3 text-xs uppercase text-[#a39e98]">Dikumpulkan</th><th class="px-4 py-3 text-xs uppercase text-[#a39e98]">Jawaban</th><th class="px-4 py-3 text-xs uppercase text-[#a39e98]">Nilai</th><th class="px-4 py-3 text-right text-xs uppercase text-[#a39e98]">Aksi</th></tr></thead>
                            <tbody class="divide-y divide-[#e6e6e6]">
                                <tr v-for="(submission, index) in props.tugas.pengumpulan_tugas ?? []" :key="submission.id" class="hover:bg-[#f6f5f4]/60">
                                ...
                                <tr v-if="!(props.tugas.pengumpulan_tugas ?? []).length"><td colspan="6" class="px-4 py-10 text-center text-sm text-[#615d59]">Belum ada mahasiswa yang mengumpulkan jawaban.</td></tr>
                            </tbody>
                        </table>
                    </div>
                </section>
