            ...
            <div class="mx-auto w-full max-w-[1000px] px-4 py-6 sm:px-6 lg:px-8">
                ...
                <div class="mb-6 flex flex-wrap items-start justify-between gap-3">
                    <div class="space-y-1">
                        ...
                        <p v-if="isMahasiswa" class="max-w-xl text-sm leading-5 text-[#615d59]">
                ...
                <form v-if="isMahasiswa" @submit.prevent="submit" class="space-y-4">
                    ...
                    >
                        ...
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-2">
                            <div class="grid gap-2">
                                <Label for="name" class="text-sm font-medium text-black">{{ labels.name }}</Label
                                ><Input id="name" v-model="form.name" :class="inp" required /><InputError :message="form.errors.name" />
                            </div>
                            <div class="grid gap-2">
                                <Label for="username" class="text-sm font-medium text-black">{{ labels.username }}</Label
                                ><Input id="username" v-model="form.username" :class="inp" required /><InputError :message="form.errors.username" />
                            </div>
                        </div>
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-2">
                            <div class="grid gap-2">
                                <Label for="email" class="text-sm font-medium text-black">{{ labels.email }}</Label
                                ><Input id="email" type="email" v-model="form.email" :class="inp" required /><InputError
                                    :message="form.errors.email"
                                />
                            </div>
                            <div class="grid gap-2">
                                <Label for="role_id" class="text-sm font-medium text-black">{{ labels.role_id }}</Label
                                ><select id="role_id" v-model="form.role_id" :class="sel" required>
                    ...
                    >
                        ...
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-2">
                            <div class="grid gap-2">
                                <Label for="tempat_lahir" class="text-sm font-medium text-black">{{ labels.tempat_lahir }}</Label
                                ><Input id="tempat_lahir" v-model="form.tempat_lahir" :class="inp" required /><InputError
                                    :message="form.errors.tempat_lahir"
                                />
                            </div>
                            <div class="grid gap-2">
                                <Label for="tanggal_lahir" class="text-sm font-medium text-black">{{ labels.tanggal_lahir }}</Label
                        ...
                        <div class="mt-4 grid gap-2">
                            ...
                            <div class="flex gap-6">
                                <label
                                    ...
                                    :key="gender"
                                    class="flex items-center gap-2 text-[15px] text-[#31302e]"
                        ...
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-2">
                            <div class="grid gap-2">
                                <Label for="agama" class="text-sm font-medium text-black">Agama</Label
                                ><select id="agama" v-model="form.agama" :class="sel" required>
                                    <option value="">Pilih agama</option>
                                    <option v-for="item in agama" :key="item" :value="item">{{ item }}</option></select
                                ><InputError :message="form.errors.agama" />
                            </div>
                            <div class="grid gap-2">
                                <Label for="no_telepon" class="text-sm font-medium text-black">{{ labels.no_telepon }}</Label
                                ><Input id="no_telepon" v-model="form.no_telepon" :class="inp" required /><InputError
                                    :message="form.errors.no_telepon"
                                />
                            </div>
                        </div>
                        <div class="mt-4 grid gap-2">
                            <Label for="kewarganegaraan" class="text-sm font-medium text-black">{{ labels.kewarganegaraan }}</Label
                            ><Input id="kewarganegaraan" v-model="form.kewarganegaraan" :class="inp" required /><InputError
                                :message="form.errors.kewarganegaraan"
                            />
                        </div>
                        <div class="mt-4 grid gap-2">
                            <Label for="alamat" class="text-sm font-medium text-black">{{ labels.alamat }}</Label
                            ><textarea
                                id="alamat"
                                v-model="form.alamat"
                                class="min-h-24 rounded-[4px] border border-[#dddddd] bg-white px-3 py-2 text-[15px] text-black placeholder:text-[#a39e98] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#0075de]"
                                required
                    ...
                    >
                        ...
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-3">
                            ...
                            <div class="grid gap-2">
                                <Label for="angkatan" class="text-sm font-medium text-black">{{ labels.angkatan }}</Label
                        ...
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-2">
                            <div class="grid gap-2">
                                <Label for="status" class="text-sm font-medium text-black">{{ labels.status }}</Label
                                ><select id="status" v-model="form.status" :class="sel" required>
                                    <option value="">Pilih status</option>
                                    <option v-for="item in statuses" :key="item" :value="item">{{ item }}</option></select
                                ><InputError :message="form.errors.status" />
                            </div>
                            <div class="grid gap-2">
                                <Label for="prodi_id" class="text-sm font-medium text-black">{{ labels.prodi_id }}</Label
                                ><select id="prodi_id" v-model="form.prodi_id" :class="sel" required>
                        ...
                        <div class="mt-4 grid gap-2">
                            ...
                            <div ref="dosenWaliRef" class="relative">
                                <button
                                    id="dosen_wali_id"
                                    type="button"
                                    class="flex h-10 w-full items-center justify-between rounded-[4px] border border-[#dddddd] bg-white px-3 text-left text-[15px] text-black"
                                    role="combobox"
                                ...
                                <div
                                    v-if="dosenWaliOpen"
                                    class="absolute z-10 mt-1 w-full rounded-xl border border-[#e6e6e6] bg-white p-2 shadow-[0_4px_18px_rgba(0,0,0,0.04),0_23px_52px_rgba(0,0,0,0.05)]"
                                    role="listbox"
                        ...
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-2">
                            <div class="grid gap-2">
                                <Label for="sekolah_asal" class="text-sm font-medium text-black">{{ labels.sekolah_asal }}</Label
                                ><Input id="sekolah_asal" v-model="form.sekolah_asal" :class="inp" required /><InputError
                                    :message="form.errors.sekolah_asal"
                                />
                            </div>
                            <div class="grid gap-2">
                                <Label for="nisn" class="text-sm font-medium text-black">{{ labels.nisn }}</Label
                                ><Input id="nisn" type="text" inputmode="numeric" v-model="form.nisn" :class="inp" required /><InputError
                                    :message="form.errors.nisn"
                                />
                            </div>
                        </div>
                        <div class="mt-4 grid gap-2">
                            <Label for="email_alternatif" class="text-sm font-medium text-black">{{ labels.email_alternatif }}</Label
                            ><Input id="email_alternatif" type="email" v-model="form.email_alternatif" :class="inp" required /><InputError
                    ...
                    >
                        ...
                        <div class="mt-4 grid gap-2">
                            <Label for="nama_ayah_kandung" class="text-sm font-medium text-black">{{ labels.nama_ayah_kandung }}</Label
                            ><Input id="nama_ayah_kandung" v-model="form.nama_ayah_kandung" :class="inp" required /><InputError
                                :message="form.errors.nama_ayah_kandung"
                            />
                        </div>
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-2">
                            <div class="grid gap-2">
                                <Label for="tanggal_lahir_ayah" class="text-sm font-medium text-black">{{ labels.tanggal_lahir_ayah }}</Label
                                ><DatePicker
                            ...
                            <div class="grid gap-2">
                                <Label for="pendidikan_terakhir_ayah" class="text-sm font-medium text-black">{{
                                    labels.pendidikan_terakhir_ayah
                                }}</Label
                                ><Input id="pendidikan_terakhir_ayah" v-model="form.pendidikan_terakhir_ayah" :class="inp" required /><InputError
                        ...
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-2">
                            <div class="grid gap-2">
                                <Label for="pekerjaan_ayah" class="text-sm font-medium text-black">{{ labels.pekerjaan_ayah }}</Label
                                ><select id="pekerjaan_ayah" v-model="form.pekerjaan_ayah" :class="sel" required>
                            ...
                            <div class="grid gap-2">
                                <Label for="penghasilan_ayah" class="text-sm font-medium text-black">{{ labels.penghasilan_ayah }}</Label
                                ><select id="penghasilan_ayah" v-model="form.penghasilan_ayah" :class="sel" required>
                        ...
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-2">
                            <div class="grid gap-2">
                                <Label for="no_telepon_ayah" class="text-sm font-medium text-black">{{ labels.no_telepon_ayah }}</Label
                                ><Input id="no_telepon_ayah" v-model="form.no_telepon_ayah" :class="inp" required /><InputError
                                    :message="form.errors.no_telepon_ayah"
                                />
                            </div>
                            <div class="grid gap-2">
                                <Label for="email_ayah" class="text-sm font-medium text-black">{{ labels.email_ayah }}</Label
                                ><Input id="email_ayah" type="email" v-model="form.email_ayah" :class="inp" required /><InputError
                                    :message="form.errors.email_ayah"
                                />
                            </div>
                        </div>
                        <div class="mt-4 grid gap-2">
                            <Label for="alamat_ayah" class="text-sm font-medium text-black">{{ labels.alamat_ayah }}</Label
                            ><textarea
                                id="alamat_ayah"
                                v-model="form.alamat_ayah"
                                class="min-h-20 rounded-[4px] border border-[#dddddd] bg-white px-3 py-2 text-[15px] text-black placeholder:text-[#a39e98] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#0075de]"
                                required
                    ...
                    >
                        ...
                        <div class="mt-4 grid gap-2">
                            <Label for="nama_ibu_kandung" class="text-sm font-medium text-black">{{ labels.nama_ibu_kandung }}</Label
                            ><Input id="nama_ibu_kandung" v-model="form.nama_ibu_kandung" :class="inp" required /><InputError
                                :message="form.errors.nama_ibu_kandung"
                            />
                        </div>
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-2">
                            <div class="grid gap-2">
                                <Label for="tanggal_lahir_ibu" class="text-sm font-medium text-black">{{ labels.tanggal_lahir_ibu }}</Label
                                ><DatePicker
                            ...
                            <div class="grid gap-2">
                                <Label for="pendidikan_terakhir_ibu" class="text-sm font-medium text-black">{{
                                    labels.pendidikan_terakhir_ibu
                                }}</Label
                                ><Input id="pendidikan_terakhir_ibu" v-model="form.pendidikan_terakhir_ibu" :class="inp" required /><InputError
                        ...
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-2">
                            <div class="grid gap-2">
                                <Label for="pekerjaan_ibu" class="text-sm font-medium text-black">{{ labels.pekerjaan_ibu }}</Label
                                ><select id="pekerjaan_ibu" v-model="form.pekerjaan_ibu" :class="sel" required>
                                    <option value="">Pilih pekerjaan</option>
                            ...
                            <div class="grid gap-2">
                                <Label for="penghasilan_ibu" class="text-sm font-medium text-black">{{ labels.penghasilan_ibu }}</Label
                                ><select id="penghasilan_ibu" v-model="form.penghasilan_ibu" :class="sel" required>
                                    <option value="">Pilih penghasilan</option>
                        ...
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-2">
                            <div class="grid gap-2">
                                <Label for="no_telepon_ibu" class="text-sm font-medium text-black">{{ labels.no_telepon_ibu }}</Label
                                ><Input id="no_telepon_ibu" v-model="form.no_telepon_ibu" :class="inp" required /><InputError
                                    :message="form.errors.no_telepon_ibu"
                                />
                            </div>
                            <div class="grid gap-2">
                                <Label for="email_ibu" class="text-sm font-medium text-black">{{ labels.email_ibu }}</Label
                                ><Input id="email_ibu" type="email" v-model="form.email_ibu" :class="inp" required /><InputError
                                    :message="form.errors.email_ibu"
                                />
                            </div>
                        </div>
                        <div class="mt-4 grid gap-2">
                            <Label for="alamat_ibu" class="text-sm font-medium text-black">{{ labels.alamat_ibu }}</Label
                            ><textarea
                                id="alamat_ibu"
                    ...
                    >
                        ...
                        <div class="mt-4 grid items-start gap-4 sm:grid-cols-2">
                            <div v-for="field in ['password', 'password_confirmation']" :key="field" class="grid gap-2">
                                <Label :for="field" class="text-sm font-medium text-black">{{ labels[field] }}</Label
                                ><Input
                                    :id="field"
                                    type="password"
                                    v-model="form[field]"
                ...
                >
                    <div class="space-y-4">
                        ...
                        <div v-for="field in roleFields" :key="field" class="grid gap-2">
                            <Label :for="field" class="text-sm font-medium text-black">{{ labels[field] }}</Label
                            ><select v-if="field === 'status'" :id="field" v-model="form[field]" :class="sel" required>
                                <option value="">Pilih status</option>
                                <option v-for="item in statuses" :key="item" :value="item">{{ item }}</option></select
                            ><select v-else-if="field === 'prodi_id'" :id="field" v-model="form[field]" :class="sel" required>
                                <option value="">Pilih program studi</option>
                                <optgroup v-for="[fakultas, prodiList] in groupedProgramStudi" :key="fakultas" :label="fakultas">
                                    <option v-for="item in prodiList" :key="item.id" :value="item.id">
                                        {{ item.jenjang }} - {{ item.nama_prodi }}
                            ...
                            ><template v-else-if="field === 'dosen_wali_id'"
                                ><div ref="dosenWaliRef" class="relative">
                                    ...
                                    <div
                                        v-if="dosenWaliOpen"
                                        class="absolute z-10 mt-1 w-full rounded-xl border border-[#e6e6e6] bg-white p-2 shadow-[0_23px_52px_rgba(0,0,0,0.05)]"
                                        role="listbox"
                                        id="dosen-wali-options"
                                    >
